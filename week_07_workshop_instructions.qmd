...
```{python}
jobs_subset["years_required"] = yrs_exp_vec(jobs_subset["job_description"])
jobs_subset.to_csv("jobs_subset.csv")
```
...
def lang_req(description):
    ...
    prompt = f"""
    Extract the programming languages from this job description. Return only `R`, `Python`, and if both languages are mentioned, return `both`. If no programming languages are mentioned, return `neither`. Here is the {description}
    """
    return llm_chat(prompt)
...
```{python}
jobs_subset["languages"] = lang_req_vec(jobs_subset["job_description"])
```
## Part B: Process Full Dataset
...
# Your code here
jobs["languages"] = lang_req_vec(jobs["job_description"])
```
